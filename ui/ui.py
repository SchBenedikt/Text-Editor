...
# Try to import Qt WebEngine; if unavailable, disable features that need it
try:
    from PyQt6.QtWebEngineWidgets import QWebEngineView
    WEB_ENGINE_AVAILABLE = True
except ImportError:
    WEB_ENGINE_AVAILABLE = False
    # Optional: log or notify later when user triggers chat feature

from PyQt6.QtPrintSupport import QPrinter, QPrintDialog
# Ensure we only import Document once from correct location
...
from ui.tools.toolbar import create_ribbon
...
class TextEditor(QMainWindow):
    def __init__(self):
        ...
        # Create and add the Ribbon
        self.ribbon = create_ribbon(self)
        self.ribbon.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        main_layout.addWidget(self.ribbon)

        # Create and add the Tab Widget for documents
        self.tab_widget = QTabWidget(self)
        self.tab_widget.setTabsClosable(True)
        self.tab_widget.tabCloseRequested.connect(self.close_tab)
        main_layout.addWidget(self.tab_widget)

        # Configure Tab Bar for adding new tabs
        self.init_tab_bar()
        self.open_empty_tab()
    ...
    def start_webserver(self):
        ...
        # Check if GitHub credentials are set in environment variables
        if not os.getenv("GITHUB_CLIENT_ID") or not os.getenv("GITHUB_CLIENT_SECRET") or not os.getenv("OAUTH_REDIRECT_URI"):
            self.show_github_credentials_dialog()
            if not os.getenv("GITHUB_CLIENT_ID") or not os.getenv("GITHUB_CLIENT_SECRET") or not os.getenv("OAUTH_REDIRECT_URI"):
                QMessageBox.warning(self, "Missing Credentials", "GitHub credentials are not set. Login functionality will not work.")
                return
    ...
    def show_github_credentials_dialog(self):
        dialog = QDialog(self)
        dialog.setWindowTitle("GitHub OAuth Credentials")
        dialog.setModal(True)
        
        layout = QVBoxLayout(dialog)
        
        client_id_label = QLabel("GitHub Client ID:")
        layout.addWidget(client_id_label)
        client_id_input = QLineEdit(dialog)
        client_id_input.setText(os.getenv("GITHUB_CLIENT_ID", ""))
        layout.addWidget(client_id_input)
        
        client_secret_label = QLabel("GitHub Client Secret:")
        layout.addWidget(client_secret_label)
        client_secret_input = QLineEdit(dialog)
        client_secret_input.setText(os.getenv("GITHUB_CLIENT_SECRET", ""))
        client_secret_input.setEchoMode(QLineEdit.EchoMode.Password)
        layout.addWidget(client_secret_input)
        
        redirect_uri_label = QLabel("Authorization Callback URL:")
        layout.addWidget(redirect_uri_label)
        redirect_uri_input = QLineEdit(dialog)
        redirect_uri_input.setText(os.getenv("OAUTH_REDIRECT_URI", "http://127.0.0.1:5000/callback"))
        layout.addWidget(redirect_uri_input)
        
        save_button = QPushButton("Save", dialog)
        save_button.clicked.connect(lambda: self.save_github_credentials(dialog, client_id_input.text(), client_secret_input.text(), redirect_uri_input.text()))
    ...
    def save_github_credentials(self, dialog, client_id, client_secret, redirect_uri):
        if client_id and client_secret and redirect_uri:
            ...
            try:
                with open('.env', 'w') as f:
                    f.write(f"# GitHub OAuth Configuration\n")
                    f.write(f"GITHUB_CLIENT_ID={client_id}\n")
                    f.write(f"GITHUB_CLIENT_SECRET={client_secret}\n")
                    f.write(f"OAUTH_REDIRECT_URI={redirect_uri}\n")
                    f.write(f"\n# Flask Configuration\n")
                    f.write(f"FLASK_SECRET_KEY={os.getenv('FLASK_SECRET_KEY', 'your_random_secret_key_here')}\n")
                QMessageBox.information(self, "Credentials Saved", "GitHub credentials have been saved.")
            except Exception as e:
                QMessageBox.warning(self, "Save Error", f"Failed to save credentials to .env file: {str(e)}")
            dialog.accept()
        else:
            QMessageBox.warning(self, "Invalid Input", "Please provide all required credentials.")
        ...
        if file:
            ...
            if content is not None:
                ...
                QMessageBox.warning(self, "Encoding Error", 
                                  f"Unable to open the file '{file}'.\n\n"
    ...
    def open_chat_tab(self):
        if not WEB_ENGINE_AVAILABLE:
            QMessageBox.warning(self, "Qt WebEngine Missing", 
                                "The Qt WebEngine module is not available.\n"
                                "Please install matching versions of PyQt6 and PyQt6-WebEngine,\n"
                                "or use the system without the Chat tab.")
            return
...
QLabel {
    ...
    # ------------------------------------------------------------------
    # New Word-like editing features
    # ------------------------------------------------------------------
    ...
    def _toggle_list(self, style):
        ...
        if isinstance(current_widget, QTextEdit):
            ...
            # Toggle: create list if not in one, otherwise remove
    ...
    def set_line_spacing(self):
        ...
        cursor = current_widget.textCursor()
        block_format = cursor.blockFormat()
        block_format.setLineSpacing(block_format.lineSpacing() + 1)
        cursor.mergeBlockFormat(block_format)
        current_widget.setFocus()
    ...
    def set_subscript(self):
        ...
        format = cursor.charFormat()
        font = format.font()
        font.setSubScript(not font.subScript())
        format.setFont(font)
        cursor.mergeCharFormat(format)
        current_widget.setFocus()
    ...
    # View modes (Read-only, Draft, Print Layout)
    def set_read_only_mode(self):
        current_widget = self.tab_widget.currentWidget()
        current_widget.setReadOnly(True)
        current_widget.setFocus()

    def set_draft_mode(self):
        current_widget = self.tab_widget.currentWidget()
        current_widget.setReadOnly(False)
        current_widget.setFocus()
    ...
    # Insert table placeholder (basic action for now)
    def insert_table_placeholder(self):
        current_widget = self.tab_widget.currentWidget()
        cursor = current_widget.textCursor()
    ...
    def show_find_replace_dialog(self):
        ...
        self.find_line_edit.setPlaceholderText("Find:")
        ...
        find_button.clicked.connect(self.perform_find)
    ...
    def perform_replace(self):
        ...
        while True:
            ...
            reply = QMessageBox.question(
                self,
                "Word Replaced",
                f"The word '{word}' was replaced with '{replacement}'.",
